  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    ...
    if (res.status === 429) {
      if (attempt === MAX_RETRIES) {
        await res.text();
        throw new Error(
          `GitLab API rate limit exceeded after ${String(MAX_RETRIES)} retries — ${url}`,
        );
      }
      const retryAfter = parseInt(res.headers.get("Retry-After") ?? "60", 10);
      if (onRetry)
        onRetry(
          `Rate limited — retrying in ${String(retryAfter)}s (attempt ${String(attempt + 1)}/${String(MAX_RETRIES)})...`,
        );
...
async function fetchAllPages(
  ...
  let nextUrl: string | null = url.includes("per_page")
    ? url
...
export function createClient(
  config: Pick<Config, "token" | "baseUrl">,
): GitLabClient {
  ...
  async function fetchProjects(
    ...
    if (projectId) {
      const res = await gitlabFetch(
        `${apiBase}/projects/${String(projectId)}`,
        token,
        onRetry,
      );
    ...
    const raw = await fetchAllPages(
      `${apiBase}/groups/${String(groupId)}/projects?include_subgroups=true`,
      token,
      onRetry,
    );
    return (raw as GitLabProject[]).map((p) => ({
      id: p.id,
      name: p.name,
  ...
  async function fetchJobsInRange(
    ...
    let nextUrl: string | null =
      `${apiBase}/projects/${String(projectId)}/jobs?per_page=100`;
    ...
    while (nextUrl) {
      ...
      for (const job of data) {
        if (!job.finished_at || job.duration == null || job.status == null)
          continue;
